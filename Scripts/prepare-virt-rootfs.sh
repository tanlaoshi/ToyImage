...
if [ -z "$HAL_ARCH" ]; then
    case "$ARCH" in
        ...
        *)
            echo "error: set TOY_VIRT_HAL_ARCH=Arm64|RiscV (or TOY_VIRT_MAKE_ARCH)" >&2
            exit 1
            ;;
    esac
fi
...
HELLO_SRC="../ToyKernel/Build/HAL/$HAL_ARCH/user/hello.elf"
if [ -f "$HELLO_SRC" ]; then
    cp -f "$HELLO_SRC" "$ROOT/HELLO.ELF"
elif [ -f "$ROOT/HELLO.ELF" ] && [ -n "$ARCH" ]; then
